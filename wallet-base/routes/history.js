...
router.get('/getHistory', async function (req, res) {
  try {
    ...
    if (!contractAddr) {
      status = 1;
      // (8) #############
      // [ request 모듈을 사용하여 address에 대한 전체 트랜잭션 히스토리 가져오기 ] http://106.10.58.158:9000/#api-Addresses-GetAddressesAddressTransactions  
      historyObj = '';
      // #############
      ...
      status = 2;
      // (9) #############
      // [ request 모듈을 사용하여 contractAddr에 대한 컨트랙트 트랜잭션 히스토리 가져오기 ]  http://106.10.58.158:9000/#api-Addresses-GetAddressesAddressTransactions
      historyObj= ''
      // #############
